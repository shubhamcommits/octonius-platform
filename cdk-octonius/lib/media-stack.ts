...
// Define interface for MediaStack props
...
// Define MediaStack class
export class MediaStack extends cdk.Stack {
    // Public properties for the media resources
    public readonly bucket: s3.Bucket
    public readonly distribution: cloudfront.Distribution


    // Constructor for the MediaStack
    constructor(scope: Construct, id: string, props: MediaStackProps) {
        super(scope, id, props)

        // Create hosted zone reference
        const hostedZone = route53.HostedZone.fromLookup(this, 'octonius_media_hosted_zone', {
            domainName: props.domain_name
        })

        // Create SSL certificate
        const certificate = new acm.Certificate(this, 'octonius_media_certificate', {
            domainName: `media.${props.domain_name}`,
            validation: acm.CertificateValidation.fromDns(hostedZone)
        })

        // Create S3 bucket for media files
        this.bucket = new s3.Bucket(this, 'octonius_media_bucket', {
            bucketName: `octonius-media-${this.account}-${this.region}`,
        ...
        this.distribution = new cloudfront.Distribution(this, 'octonius_media_distribution', {
            defaultBehavior: {
            ...
            domainNames: [`media.${props.domain_name}`],
        ...
        // Create DNS record
        new route53.ARecord(this, 'octonius_media_dns', {
            zone: hostedZone,
            target: route53.RecordTarget.fromAlias(
                new targets.CloudFrontTarget(this.distribution)
            ),
            recordName: `media.${props.domain_name}`
        })

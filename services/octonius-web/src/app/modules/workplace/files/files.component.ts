...
export class FilesComponent implements OnInit, OnDestroy {
  ...
  private async loadData(): Promise<void> {
    try {
      ...
      if (!groupId) {
        ...
        this.toastService.error('No group selected. Please select a group first.');
        return;
      }

      // Load group data
      const group = await firstValueFrom(this.workGroupService.getGroup(groupId));
      this.group = group;
  ...
  private async loadGroupFiles(): Promise<void> {
    ...
    try {
      ...
      this.error = 'Failed to load group files. Please try again.'
      this.isLoading = false
      this.toastService.error('Failed to load group files. Please try again.')
  ...
  async onCreateNote(): Promise<void> {
    ...
    try {
      ...
      const newNote = await firstValueFrom(this.fileService.createGroupNote(noteData));
  ...
  private async uploadFile(file: globalThis.File): Promise<void> {
    ...
    try {
      ...
      const response = await firstValueFrom(this.fileService.uploadGroupFileS3(file, this.group.uuid))
